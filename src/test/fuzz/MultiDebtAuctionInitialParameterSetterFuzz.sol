...
import "./MultiDebtAuctionInitialParameterSetterMock.sol";
...
contract FuzzBounds is MultiDebtAuctionInitialParameterSetterMock {

    TokenMock systemCoin;
    uint256 _coinsToMint = 1E40;

    constructor() MultiDebtAuctionInitialParameterSetterMock(
            bytes32("BAI"),
            address(new Feed(1000 ether, true)),
            address(new Feed(2.015 ether, true)),
            address(new AccountingEngine()),
            address(new MultiMockTreasury(address(new TokenMock()))),
            3600, // periodSize
        ...
        systemCoin = TokenMock(address(treasury.systemCoin("BAI")));

        MultiMockTreasury(address(treasury)).setTotalAllowance("BAI", address(this), uint(-1));
        MultiMockTreasury(address(treasury)).setPerBlockAllowance("BAI", address(this), 10E45);
...
contract Fuzz is MultiDebtAuctionInitialParameterSetterMock {
